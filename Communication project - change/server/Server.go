...
func PrintNetErr(oper string, err error) {
	if err == nil {
		return
	}
	var opErr *net.OpError
	if errors.As(err, &opErr) {
		fmt.Printf("[网络故障]%s 操作失败：远程连接异常\n", oper)
		return
	}
	if errors.Is(err, context.DeadlineExceeded) {
		fmt.Printf("[超时]%s：客户端长时间无交互\n", oper)
		return
	}
	if errors.Is(err, io.EOF) {
		fmt.Printf("[正常下线]%s：客户端主动关闭\n", oper)
		return
	}
	fmt.Printf("[未知错误]%s：%v\n", oper, err)
}
...
func (s *Server) handleConn(conn net.Conn) {
	...
	*/
	...
	go func() { // 为当前用户单独启动读取消息协程
		// 新增：创建bufio.Reader封装用户连接
		reader := bufio.NewReader(user.conn)
		//新 给这条 TCP 连接所有读操作设置截止时间点。
		//只要到了设置的时间，任何读操作（Read / ReadString）都会立刻返回错误 context.DeadlineExceeded，不会继续阻塞卡死。每次调用该函数SetReadDeadline都会刷新截止时间点。
		_ = user.conn.SetReadDeadline(time.Now().Add(60 * time.Second))
		for { // 无限循环，持续等待客户端发数据
			//旧msg := make([]byte, 4096)     // 1. 开辟4096字节缓冲区，用来存放客户端发来的二进制数据
			//旧n, err := user.conn.Read(msg) // 2. 阻塞读取TCP连接数据：没有数据就卡在这等待，不占用CPU
			//新 按行读取，直到\n结束，自动处理粘包
			msg, err := reader.ReadString('\n') //读取一整行消息。阻塞读取TCP连接数据：没有数据就卡在这等待，不占用CPU
			if err != nil {
				// 两种情况会进这里：
				// ①客户端主动关闭：io.EOF
				// ②60秒无任何数据，读超时：context.DeadlineExceeded
				PrintNetErr("读取客户端消息", err)
				//用户业务封装环节，代码替换
			...
			//旧message := string(msg[:n]) // 3. 截取有效数据：msg缓冲区是4096字节，只用前n个收到的字节
			//新 剔除首尾空白（换行、空格），得到纯净的业务消息
			message := strings.TrimSpace(msg)
			if message == "" { // 空消息直接跳过
				continue
			}

			// 只要收到任意业务消息，立刻刷新60秒倒计时
			_ = user.conn.SetReadDeadline(time.Now().Add(60 * time.Second))

			//用户业务封装环节，代码替换
			//s.Broadcast(user,message)  // 调用全局广播，把这条用户输入的消息推送给所有在线客户端
			//改为：
